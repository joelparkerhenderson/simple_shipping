...
*Example*
    fedex_client.request(shipper, recipient, package, :extra_opts => {:dropoff_type => :drop_box})

=== UPS extra options
* :service_type (see SimpleShipping::Ups::ShipmentBuilder::SERVICE_TYPES for available values)

*Example*
    ups_client.request(shipper, recipient, package, :extra_opts => {:service_type => :express})
